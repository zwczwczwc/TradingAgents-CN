...
def run_analysis(token, symbol, depth, analysts=None):
    """提交分析任务"""
    ...
    if analysts:
        data["parameters"]["analysts"] = analysts

    print(f"🚀 提交指数分析任务: {symbol} (类型: index, 深度: {depth})...")
    if analysts:
        print(f"👥 指定分析师: {analysts}")
...
def main():
    ...
    parser.add_argument('--index', help='指数名称或代码 (例如: 半导体, sh000001)')
    parser.add_argument('--symbol', help='指数代码 (同 --index)')
    parser.add_argument('--output', default='report.pdf', help='输出文件名 (默认: report.pdf)')
    parser.add_argument('--depth', default='深度', choices=['快速', '基础', '标准', '深度', '全面', '深快速'], help='研究深度 (默认: 深度)')
    parser.add_argument('--analysts', nargs='+', help='指定分析师模块 (例如: macro policy news sector technical)')
    
    args = parser.parse_args()
    
    if not args.index and not args.symbol:
        parser.error("必须提供 --index 或 --symbol 参数")
        
    target_index = args.index if args.index else args.symbol
    ...
    print("=" * 50)
    print(f"📊 开始指数分析任务")
    print(f"🎯 目标指数: {target_index}")
    print(f"🔍 分析深度: {args.depth}")
    print(f"📂 保存路径: {output_path}")
    print("=" * 50)
    
    token = login_or_register()
    task_id = run_analysis(token, target_index, args.depth, args.analysts)
